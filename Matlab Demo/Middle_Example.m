function[IPP,TE,TI,TR,TNumPatches,TimeEnd,TotalUniqueE]=...
Middle_Example(NR,E0,A,M,L,mu,rho,alpha,gamma,beta,Q,PopDist)
...
TE(j,1:length(TNewE)) = TNewE;
TR(j,1:length(TNewR)) = TNewR;
SPPI = sum(PPNewI);
SPPI(SPPI~=0)=1;
IPP(j,:) = SPPI;
